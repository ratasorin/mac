function sol = gauss_seidel_solve_sistem (A, b, steps) 
    ...
    for i = 1 : length(A)
        ...
        if(element < sum) 
            error = MException("gauss_seidel_solve_sistem:not_diagonally_dominant", "The matrix is not diagonally dominant! On line '%d' the element '%d' is lower than the sum '%d'!", i, element, sum);
            throw(error);
        end
    end 
